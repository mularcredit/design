import requests
import json
import os
import time

HOST = 'https://mularcredit.co.ke:2083'
USER = 'mular'
TOKEN = 'M8GJSJK5MRU8EITT8OQ9J5SBU2423V38'
headers = {'Authorization': f'cpanel {USER}:{TOKEN}'}

TARGET_DOMAIN = 'centralbank.go.ke'

def exhaustive_search(email_user):
    print(f"\nExhaustive scan for {email_user}@mularcredit.co.ke ...")
    sent_path = f"mail/mularcredit.co.ke/{email_user}/.Sent/cur"
    
    r = requests.get(f"{HOST}/execute/Fileman/list_files", headers=headers, params={
        "dir": sent_path,
        "showhidden": 1
    }, timeout=30)
    
    if r.status_code != 200 or r.json().get("status") != 1:
        print(f"  Sent folder not accessible or empty.")
        return False

    files = r.json().get("data", [])
    print(f"  Found {len(files)} sent messages. Scanning all...")
    
    # Sort by mtime (most recent first)
    files.sort(key=lambda x: x.get("mtime", 0), reverse=True)
    
    # Check all files
    for idx, f_info in enumerate(files):
        filename = f_info.get("file")
        try:
            r_file = requests.get(f"{HOST}/execute/Fileman/get_file_content", headers=headers, params={
                "dir": sent_path,
                "file": filename
            }, timeout=20)
            
            if r_file.status_code == 200:
                data_dict = r_file.json().get("data") or {}
                content = data_dict.get("content", "")
                
                if TARGET_DOMAIN.lower() in content.lower():
                    print(f"\n[FOUND MATCH!] In {email_user} Sent")
                    print(f"File: {filename}")
                    print(f"Date: {time.ctime(f_info.get('mtime'))}")
                    
                    headers_found = {}
                    for line in content.splitlines():
                        for h in ["Date:", "From:", "To:", "Subject:"]:
                            if line.startswith(h):
                                headers_found[h] = line
                    for h in ["Date:", "From:", "To:", "Subject:"]:
                        if h in headers_found:
                            print(f"  {headers_found[h]}")
                    return True
            
            if (idx + 1) % 100 == 0:
                print(f"    ... {idx + 1}/{len(files)} checked")
                
        except Exception:
            pass
    return False

def main():
    # Priority list based on high sent counts from find_active_sent.py
    priority_users = ['hr', 'jojijo', 'jlui']
    for user in priority_users:
        if exhaustive_search(user):
            print("\nMatch found. Stopping.")
            return
    print("\nExhaustive search completed without match.")

if __name__ == "__main__":
    main()
